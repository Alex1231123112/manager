...
5. **В админке:** выбери команду (или создай её в боте). Доступны: Дашборд, Матчи, Календарь, Таблица, Долги, Финансы, **Участники** (имя, @username, роли, участие в матчах), **Приглашения** (ссылка и QR), **Интеграция** (метрики доставки сообщений в Telegram), **Настройки** (канал для постов; Telegram администратора в формате @username).
...
- **admin-ui** — Next.js, порт 3000; запросы к API проксируются на `app:8080`. После доработок фронта пересоберите образ, иначе на вебе будет старая версия: `docker compose build admin-ui`, затем `docker compose up -d admin-ui` (или перезапустите весь стек).
...
**Админка с автоперезагрузкой при изменении файлов** (режим разработки):

```bash
docker compose -f docker-compose.yml -f docker-compose.dev.yml up --build
```

Зависимости (`node_modules`) ставятся в контейнере при первом запуске — на хосте ничего устанавливать не нужно. Обязательно оба файла и `--build`. Фронт подхватывает изменения в `admin-ui/` (Fast Refresh); бэкенд и БД — как в обычном `docker compose up`.

Если в dev-режиме появляется ошибка `Cannot find module '../lightningcss.linux-x64-musl.node'`, значит в томе лежат старые `node_modules` (с хоста или под другую ОС). Удалите том и пересоберите:

```bash
docker compose -f docker-compose.yml -f docker-compose.dev.yml down
docker volume rm manager_admin_ui_node_modules
docker compose -f docker-compose.yml -f docker-compose.dev.yml up --build
```

(Имя тома может быть с префиксом проекта, например `manager_admin_ui_node_modules`; при необходимости посмотрите список: `docker volume ls`.)
...
├── src/main/resources/db/migration/  — Flyway (V1–V18: схема, приглашения, явка, статистика, финансы, события, интеграция)
