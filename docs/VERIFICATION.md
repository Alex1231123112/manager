...
Миграции БД (Flyway V1–V10) применяются при старте контейнера **app**. V9 — `team_members.is_active`, V10 — роли только ADMIN/PLAYER; V8 — `telegram_username`, `display_name`, `admin_telegram_username`.
...
| Шаг | Действие | Ожидаемый результат |
|-----|----------|---------------------|
| 3.1 | В админке **Приглашения** → создать приглашение (роль Игрок или Админ), скопировать ссылку или показать QR. | Ссылка вида `t.me/BotUsername?start=КОД`. |
| 3.2 | Новый пользователь переходит по ссылке или в боте вводит **/start КОД**. | «Вы добавлены в команду «…». Роль: … Если бот в чате команды, напиши там /start.» |
| 3.3 | Повторно **/start КОД** или неверный код. | «Приглашение недействительно или уже использовано.» |
