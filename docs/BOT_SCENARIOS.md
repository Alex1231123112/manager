...
|-----|------------------------|------------|
| 1 | Отправляет `/start` | «Привет! Отправь название команды одним сообщением. Например: БК Метеор» |
...
| Действие | Результат |
|----------|-----------|
| Кнопка **Долги** или `/debt` | Если есть должники: список «• Имя — сумма ₽» и подсказка «Изменить: /setdebt Имя Сумма». Если долгов нет: «Долгов нет.» |
...
| Callback от неизвестной кнопки (не `publish:…`) | Ничего не делается. |

---

## 9. Сводка: все команды через Reply- или Inline-кнопки

**Reply-кнопки (главное меню):** Состав, Добавить игрока, Новый матч, Ввести результат, Опрос на игру, Долги, **Команды**.
...
Все сценарии предполагают, что команда уже создана (чат привязан к команде). Иначе бот отвечает: «Сначала создай команду: отправь /start и затем название команды.»
